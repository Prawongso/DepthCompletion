...
class Pluralistic(BaseModel):
    ...
    def __init__(self, opt):
        """Initialize the pluralistic model"""
        BaseModel.__init__(self, opt)

        self.loss_names = ['kl_rec', 'kl_g', 'app_rec', 'app_g', 'ad_g', 'img_d']
    ...
    def backward_D(self):
        """Calculate GAN loss at multi-scales"""
        self.loss_img_d = self.backward_D_basic(self.net_D, self.img_truth, self.img_out)

    def backward_G(self):
        """Calculate GAN and L1 loss for the generator"""
        # GAN loss
        self.loss_ad_g = self.GANloss(self.net_D(self.img_out), True, False) * self.opt.lambda_g
        # L1 loss
        self.loss_app_rec = 0
        self.loss_app_g = 0
        for i in range(len(self.img_g)):
            self.loss_app_rec += self.L1loss(self.img_rec[i], self.scale_img[i]) * self.opt.lambda_rec
            self.loss_app_g += self.L1loss(self.img_g[i], self.scale_img[i]) * self.opt.lambda_rec
        # kl loss
        self.loss_kl_rec = self.kl_rec * self.opt.lambda_kl
        self.loss_kl_g = self.kl_g * self.opt.lambda_kl

        total_loss = self.loss_ad_g + self.loss_app_g + self.loss_kl_g + self.loss_app_rec + self.loss_kl_rec
        total_loss.backward()
